...
select 
    ...
    then orders.order_value else 0 end) as total_lifetime_value,

    sum(case when orders.valid_order_date is not null 
    then orders.order_value else 0 end)/nullif(count(case when orders.valid_order_date is not null 
